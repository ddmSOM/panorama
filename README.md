...
```
"dependencies": {
    "@stamen/panorama": "americanpanorama/panorama",
    ...
}
```
...
Finally, switch over to your application that is using `@stamen/panorama` and uninstall and reinstall the toolkit with these commands:

```
npm uninstall @stamen/panorama
npm install @stamen/panorama
```
...
*Before running this script, you'll have to be logged into npm with username: `panorama` / email: `ericsoco@stamen.com`.* Contact a `@stamen/panorama` administrator for the password. You can either login via [`npm login`](https://docs.npmjs.com/cli/adduser) or via [npm's website](https://www.npmjs.com/login).
